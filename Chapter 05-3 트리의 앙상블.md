...
> **그레디언트 부스팅(gradient boosting)**: 깊이가 얕은 결정 트리를 사용해 이전 트리의 오차를 보완하는 방식으로 앙상블 하는 방법이다. 경사 하강법을 사용해 트리를 앙상블에 추가한다.  
> 초기에 상수 함수 값을 생성하면, 이후 트리(Fn, n >= 1)부터는 현재 예측값에 대한 손실함수의 음의 gradient를 근사한다.  
> 해당 지점의 음의 gradient를 학습률과 곱하여 초기값에 더함으로써 보정을 수행한다. 보정, 음의 gradient, 학습률을 각각 이동, 이동 방향, 보폭이라고 이해하면 쉽다.
> Gradient Boosting은 음의 gradient를 근사하는 모델을 학습하고, 그 값을 예측에 더함으로써 손실이 감소하는 방향으로 예측을 점진적으로 보정한다.
